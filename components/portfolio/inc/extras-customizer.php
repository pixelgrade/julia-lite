 ...
 * @return string
 */
function pixelgrade_portfolio_grid_vertical_spacing_cb( $value, $selector, $property, $unit ) {
	...
	if ( ! empty( $typeline_config['spacings']['points'] ) && ! empty( $typeline_config['spacings']['breakpoints'] ) ) {
		...
		// from 80em
		$columns  = pixelgrade_option( 'portfolio_items_per_row', 3 );
...
function pixelgrade_portfolio_grid_horizontal_spacing_cb( $value, $selector, $property, $unit ) {
	$output = '';
 ...
 */
function pixelgrade_portfolio_grid_vertical_spacing_cb_customizer_preview() {
	$js = '';
...
add_action( 'customize_preview_init', 'pixelgrade_portfolio_grid_vertical_spacing_cb_customizer_preview', 20 );

/**
 * Inline enqueues the JS code used in the Customizer for the portfolio grid spacing live preview.
 */
function pixelgrade_portfolio_grid_horizontal_spacing_cb_customizer_preview() {
	$js = '';
...
/**
 * Decides when to show the portfolio grid title nearby alignment control.
 *
 * @return bool
 */
...
/**
 * Decides when to show the portfolio grid title overlay alignment control.
 *
 * @return bool
 */
...
/**
 * Decides when to show the portfolio grid items aspect ratio control.
 *
 * @return bool
 */
